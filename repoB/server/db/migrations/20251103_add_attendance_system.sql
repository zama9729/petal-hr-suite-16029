...
-- Update timesheet_entries to support attendance source
-- Make timesheet_id optional for attendance-only entries
ALTER TABLE timesheet_entries 
ALTER COLUMN timesheet_id DROP NOT NULL,
ADD COLUMN IF NOT EXISTS source TEXT CHECK (source IN ('api', 'upload', 'manual')) DEFAULT 'manual',
ADD COLUMN IF NOT EXISTS attendance_event_id UUID,
ADD COLUMN IF NOT EXISTS attendance_upload_row_id UUID,
ADD COLUMN IF NOT EXISTS start_time_utc TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS end_time_utc TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS payroll_status TEXT DEFAULT 'pending_for_payroll' CHECK (payroll_status IN ('pending_for_payroll', 'processed', 'excluded')),
...
-- Updated at trigger for attendance_uploads
